    ...
    if chat.type == 'private':
        if is_owner(user.id):
            ...
            await update.message.reply_text(
                ...
                "Bot guruhlarda faqat kanalga obuna bo'lgan foydalanuvchilarga yozishga ruxsat beradi."
            )
    else:
        try:
...
# ------------------------------
# Handle message va ogohlantirish
# ------------------------------
    ...
    if not is_member:
        try:
            ...
            warning_msg = await update.message.reply_text(
                f"👋 {user.mention_html()}\n❗️ Iltimos, avval {settings['channel_username']} kanaliga obuna bo'ling, keyin yozishingiz mumkin!",
                parse_mode="HTML"
            )
            # 10 soniyadan keyin xabar o'chadi
            context.job_queue.run_once(
                lambda ctx: ctx.bot.delete_message(warning_msg.chat_id, warning_msg.message_id),
                10
            )
        except Exception as e:
