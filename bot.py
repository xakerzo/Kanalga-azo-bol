...
# Kanal a'zoligini tekshirish - YANGILANDI
async def check_channel_membership(user_id, channel_username, context):
    try:
        if not channel_username.startswith('@'):
            channel_username = '@' + channel_username
        
        # Avval kanal mavjudligini tekshirish
        try:
            await context.bot.get_chat(channel_username)
        except Exception as e:
            logger.error(f"Kanal topilmadi: {channel_username}, xato: {e}")
            return True  # Kanal topilmasa, barchaga ruxsat ber
        
        # A'zolikni tekshirish
        member = await context.bot.get_chat_member(channel_username, user_id)
        is_member = member.status in ['member', 'administrator', 'creator']
        logger.info(f"User {user_id} in {channel_username}: {is_member} (status: {member.status})")
        return is_member
    except Exception as e:
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # DEBUG: Keling, avval qanday guruhlar borligini ko'ramiz
    conn = sqlite3.connect('channel_bot.db')
    cursor = conn.cursor()
    ...
    logger.info(f"Broadcast uchun {len(groups)} ta guruh topildi: {groups}")
    
    if not groups:
        await update.message.reply_text("❌ Hech qanday guruh topilmadi! Avval botni guruhga qo'shing va sozlang.")
        return
    ...
    for group in groups:
        ...
        try:
            ...
            sent_count += 1
            logger.info(f"✅ Xabar guruh {group_id} ga yuborildi")
        except Exception as e:
            logger.error(f"❌ Guruh {group_id} ga xabar yuborish xatosi: {e}")
            failed_count += 1
            failed_groups.append(f"{group_id}: {str(e)}")
    
    result_text = f"✅ Xabar yuborildi!\n✔️ Muvaffaqiyatli: {sent_count}\n❌ Xatolik: {failed_count}"
    
    if failed_groups:
        result_text += f"\n\nXato bo'lgan guruhlar:\n" + "\n".join(failed_groups[:5])  # Faqat birinchi 5 tasi
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # DEBUG log
    logger.info(f"Start command: user_id={user.id}, chat_id={chat.id}, chat_type={chat.type}")
    ...
    if chat.type == 'private':
        ...
        try:
            ...
            if bot_member.status not in ['administrator', 'creator']:
                await update.message.reply_text(
                    "⚠️ Botga admin huquqlarini bering!\n\n"
                    "Bot to'liq ishlashi uchun quyidagi huquqlar kerak:\n"
                    "• Xabarlarni o'chirish\n• Xabar yuborish\n• Boshqalar bilan birgalikda ishlash"
                )
                return
        except Exception as e:
            logger.error(f"Bot adminlik tekshirish xatosi: {e}")
...
# Xabarlarni qayta ishlash - YANGILANDI
    ...
    if 'waiting_for_channel' in context.user_data and context.user_data['waiting_for_channel']:
        ...
        try:
            ...
            await update.message.reply_text(
                ...
                f"Endi faqat {channel_username} kanaliga obuna bo'lgan foydalanuvchilar guruhda yozishi mumkin."
            )
        ...
        logger.info(f"User {user.id} in {chat.id}: is_member={is_member}, channel={settings['channel_username']}")
        ...
        if not is_member:
            try:
                ...
                try:
                    ...
                    logger.info(f"✅ Xabar o'chirildi: {user.id} in {chat.id}")
                except Exception as delete_error:
                    logger.error(f"❌ Xabarni o'chirish mumkin emas: {delete_error}")
                    # Xabarni o'chirish mumkin bo'lmasa ham ogohlantirish yuboramiz
                ...
                logger.info(f"✅ Ogohlantirish yuborildi: {user.id} in {chat.id}")
                ...
                async def delete_warning(context: ContextTypes.DEFAULT_TYPE):
                    try:
                        ...
                        logger.info(f"✅ Ogohlantirish o'chirildi: {warning_msg.message_id}")
                    except Exception as e:
