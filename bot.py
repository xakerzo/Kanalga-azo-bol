    ...
    if chat.type == 'private':
        if is_owner(user.id):
            ...
            await update.message.reply_text(
                ...
                "Botni guruhga qo‘shib, /settings ni bosing."
            )
    else:
        try:
            member = await chat.get_member(user.id)
            if member.status not in ['administrator', 'creator']:
                await update.message.reply_text("❌ Faqat adminlar sozlashi mumkin!")
                return
        except:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "add_channel":
        ...
        await query.edit_message_text(
            "✏️ Obuna bo'lmaganda chiqadigan xabarni yozing:"
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('waiting_for_channel'):
        ...
        if not channel_username.startswith('@'):
            await update.message.reply_text("❌ @ bilan yuboring!")
            return
        ...
        try:
            ...
            try:
                await context.bot.delete_message(
                    chat.id,
                    context.user_data['editing_message_id']
                )
            except:
                pass
            
            await update.message.reply_text(
                f"✅ Sozlandi! Endi faqat {channel_username} obunachilari yozadi."
            )
        ...
        if settings:
            ...
            await update.message.reply_text("❌ Avval kanalni sozlang!")
...
def main():
    ...
    print("🚀 Railway serverda bot ishga tushdi!")
    print("👑 Owner ID:", BOT_OWNER_ID)
