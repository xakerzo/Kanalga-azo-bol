...
# Broadcast komandasi - YANGILANDI
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ma'lumotlar bazasidan guruhlarni olish
    conn = sqlite3.connect('channel_bot.db')
    cursor = conn.cursor()
    ...
    logger.info(f"📢 Broadcast: {len(groups)} ta guruh topildi")
    
    if not groups:
        await update.message.reply_text(
            "❌ Hech qanday guruh topilmadi!\n\n"
            "Bot hali hech qanday guruhda sozlanmagan. "
            "Avval botni guruhga qo'shing va /settings orqali kanal sozlang."
        )
        return
    ...
    for group in groups:
        group_id = group[0]
        try:
            # Guruhda bot mavjudligini tekshirish
            try:
                ...
                if chat_member.status not in ['administrator', 'creator', 'member']:
                    failed_details.append(f"❌ {group_id}: Bot guruhda admin/member emas")
                    failed_count += 1
                    continue
            except Exception as e:
                failed_details.append(f"❌ {group_id}: Bot guruhda mavjud emas - {str(e)}")
                failed_count += 1
                continue
    ...
    # Xato tafsilotlarini qo'shish (faqat birinchi 3 tasi)
    if failed_details:
        result_text += f"\n\nXatolar (dastlabki 3 tasi):\n" + "\n".join(failed_details[:3])
    
    await update.message.reply_text(result_text)
    logger.info(f"📊 Broadcast yakunlandi: {sent_count} muvaffaqiyatli, {failed_count} xatolik")

# Stats komandasi - YANGILANDI
    ...
    if 'waiting_for_channel' in context.user_data and context.user_data['waiting_for_channel']:
        ...
        if not is_member:
            try:
                ...
                # Oddiy xabar yuborish (reply emas)
